...
type Props = {
  tags: string[]
  data: CollectionEntry<"certificates">[]
}

export default function Blog({ data, tags }: Props) {
  const [filter, setFilter] = createSignal(new Set<string>())
  const [certificates, setCertificates] = createSignal<CollectionEntry<"certificates">[]>([])

  createEffect(() => {
    setCertificates(data.filter((entry) =>
      ...
      <div class="col-span-3 sm:col-span-2">
        <div class="flex flex-col">
          <div class="text-sm uppercase mb-2">
            SHOWING {certificates().length} OF {data.length} CERTIFICATES
          </div>
          <ul class="flex flex-col gap-3">
            {certificates().map((certificate) => (
              <li>
                <CertCard entry={certificate} />
              </li>
            ))}
          </ul>
        </div>
